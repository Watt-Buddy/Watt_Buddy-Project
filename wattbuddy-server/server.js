...
app.post('/api/esp32/data', async (req, res) => {
  try {
    ...
    // Handle relay values sent as booleans, strings, or numbers.
    const parseRelayState = (value) => {
    ...
    const parsedRelay1 = parseRelayState(relay1);
    const parsedRelay2 = parseRelayState(relay2);
        ...
        try {
          ...
          if (baseline.isInGracePeriod) {
            ...
            if (shouldAlert) {
              ...
              let detailMsg = '';
              if (patternDeviation) {
                detailMsg = ` Expected ~${parseFloat(patternDeviation.expectedAvg || 0).toFixed(0)}W for this time, got ${currentPower.toFixed(0)}W (z=${parseFloat(patternDeviation.zScore || 0).toFixed(2)})`;
              } else {
                const anomalyThreshold = parseFloat(anomaly.threshold || baseline.threshold || 0);
                const anomalyZScore = parseFloat(anomaly.zScore || 0);
                detailMsg = ` (normal=${parseFloat(baseline.mean || 0).toFixed(0)}W, threshold=${anomalyThreshold.toFixed(0)}W, z=${anomalyZScore.toFixed(2)})`;
              }
              ...
              const anomalyData = {
                ...
                anomalySource,
                currentPower,
                threshold: parseFloat(anomaly.threshold || baseline.threshold || 0),
                userMean: parseFloat(baseline.mean || 0),
                zScore: patternDeviation ? parseFloat(patternDeviation.zScore || 0) : parseFloat(anomaly.zScore || 0),
